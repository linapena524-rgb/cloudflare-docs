...
| 10058 | TooManyRequests | 429 | Rate limit exceeded. For object writes, requests to the same key above the supported rate of one write per second may return this error. | Reduce the request rate. For same-key writes, serialize updates. Refer to [R2 limits](/r2/platform/limits/). |
